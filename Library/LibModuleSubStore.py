...
def receiveInventoryDispatch(danpheEMR, substore, ssReqNo):
      ...
      time.sleep(3)
      danpheEMR.find_element_by_link_text("Core CFG Parameters").click()
      time.sleep(3)
      danpheEMR.find_element_by_id("quickFilterInput").send_keys("EnableReceivedItemInSubstore")
      time.sleep(3)
      EnableReceivedItemInSubstore = danpheEMR.find_element_by_xpath("(//div[@col-id='ParameterValue'])[2]").text
      if EnableReceivedItemInSubstore == 'true':
            danpheEMR.find_element_by_link_text("SubStore").click()
            time.sleep(5)
